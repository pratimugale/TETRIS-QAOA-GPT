...
# make nanoGPT and source directories importable
script_dir = os.path.dirname(os.path.abspath(__file__))
src_qaoa_gpt = os.path.abspath(os.path.join(script_dir, ".."))
project_root = os.path.abspath(os.path.join(src_qaoa_gpt, "../..")) # Go up from src/qaoa-gpt to the repo root

sys.path.insert(0, src_qaoa_gpt)
...
for L, inst_indices in len_buckets.items():
    ...
    for i in range(0, len(inst_indices), args.batch_size):
        ...
        # Max context=context window set in model config, generate for remaining space
        max_gen = model.config.block_size - L
        ...
        batch_latency = total_batch_time / B
        time_per_circuit = total_batch_time / (B * args.n_samples)
...
circuit_times   = np.array([e.get('time_per_circuit', 0.0) for e in all_samples])
adapt_runtimes  = np.array([e.get('adapt_runtime', 0.0) for e in all_samples])

avg_inf   = inference_times.mean()
avg_circ  = circuit_times.mean()
avg_adapt = adapt_runtimes.mean()
speedup   = avg_adapt / avg_inf if avg_inf > 0 else 0

print()
print(f"{'Avg Inf. Time (per formula)':<30} {avg_inf:>10.4f}s")
print(f"{'Avg Inf. Time (per circuit)':<30} {avg_circ:>10.4f}s")
...
summary_df   = pd.DataFrame({
    ...
    'inference_time_per_formula': inference_times,
    'inference_time_per_circuit': circuit_times,
