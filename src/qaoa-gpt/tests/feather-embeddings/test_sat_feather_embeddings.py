...
# Add project root as well to find FEATHER
...
def main():
    ...
    # Setup results directory local to this script
    results_dir = Path(__file__).resolve().parent / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    embedder = SATGraphEmbedder(n_nodes=n_vars)
    ...
    print("Generating 100 random different formulas with +/- 20% variation in n_clauses...")
    random_formulas = []
    for _ in range(100):
        # Sample around 4.26 * n_vars with 20% variation
        rand_clauses = int(round(n_clauses * random.uniform(0.8, 1.2)))
        random_formulas.append(generate_random_3sat(n_vars, rand_clauses))
    ...
    plt.legend()
    plt.title(f"Perturbation Analysis: T-SNE (Perplexity=30) (N={n_vars}, M={n_clauses})")
    plt.xlabel("TSNE Dimension 1")
    plt.ylabel("TSNE Dimension 2")
    plt.grid(True, linestyle='--', alpha=0.6)
    
    save_path_tsne = str(results_dir / "sat_sensitivity_tsne.png")
    ...
    plt.legend()
    plt.title(f"Perturbation Analysis: PCA (Linear Projection) (N={n_vars}, M={n_clauses})")
    plt.xlabel("Principal Component 1")
    plt.ylabel("Principal Component 2")
    plt.grid(True, linestyle='--', alpha=0.6)
    
    save_path_pca = str(results_dir / "sat_sensitivity_pca.png")
    ...
    # Generate Box Plot
    plt.figure(figsize=(10, 8))
    plt.boxplot(all_dists, tick_labels=group_labels, patch_artist=True, 
                boxprops=dict(facecolor='lightblue', color='blue'),
                medianprops=dict(color='red'))
    
    plt.title(f"Perturbation Analysis: Distance from Base Formula (Euclidean) (N={n_vars}, M={n_clauses})")
    plt.ylabel("Euclidean Distance")
    plt.grid(True, linestyle='--', alpha=0.6)
    
    save_path_box = str(results_dir / "sat_sensitivity_boxplot.png")
